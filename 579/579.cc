...
double GetAngleBetweenAngles(double angle1, double angle2)
{
...
double GetAngleOfHour(int H, int M)
{
    double baseAngle = 360.0 * (H / 12.0);

    double extraAngleToAdd = 360.0 / 12.0 * (M / 60.0);

    return baseAngle + extraAngleToAdd;
}

double GetAngleOfMinute(int M)
{
    return 360.0 * (M / 60.0);
}
...
{
    double hourAngle = GetAngleOfHour(H, M);
    double minuteAngle = GetAngleOfMinute(M);

    return GetAngleBetweenAngles(hourAngle, minuteAngle);
}

void GetHAndM(int &H, int &M)
{
    string line;
...
int main()
{
    while (true)
    {
        ...
        cout << fixed << setprecision(3) << GetAngle(H, M) << endl;
    }
